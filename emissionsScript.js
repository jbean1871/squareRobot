...
function inputs() {
  ...
  ---------------------------------------------------------*/
	...
	const fuels = {
		diesel: [0.01, 130.0, 188.0, 12.101, 8907.0, 7.1, 86.5, molsCarbonPerGFuel, molsCarbonPerGFuel], // 5
		jetFuel: [0.01, 130.0, 162.0, 12.39, 8933.0, 7.0, 86.5, molsCarbonPerGFuel, molsCarbonPerGFuel], //6
		kerosene: [0.01, 130.0, 162.0, 12.39, 8933.0, 7.0, 86.5, molsCarbonPerGFuel, molsCarbonPerGFuel], //7
		gasoline: [5.2, 66.0, 92.0, 11.724, 5237.3, 5.6, 86.5, molsCarbonPerGFuel, molsCarbonPerGFuel], //8
		motorGasolineRVP13: [7.0, 62.0, 92.0, 11.644, 5043.6, 5.6, 86.5, molsCarbonPerGFuel, molsCarbonPerGFuel], //9
		motorGasolineRVP10: [5.2, 66.0, 92.0, 11.724, 5237.3, 5.6, 86.5, molsCarbonPerGFuel, molsCarbonPerGFuel], //10
		motorGasolineRVP7: [3.5, 68.0, 92.0, 11.833, 5500.6, 5.6, 86.5, molsCarbonPerGFuel, molsCarbonPerGFuel], //10
	};
  ...
  ---------------------------------------------------------*/
	...
	let pa_maxTankVol =
		i_roofType == "Floating"
			? pa_tankBottomArea * pa_tankHeight
			: pa_tankBottomArea * (pa_tankHeight + (pa_roofSlope * pa_tankDiameter) / 6); //12
	let pa_vaporSpaceVol =
		pa_roofType == "Fixed"
			? pa_vaporSpaceOutage * ((Math.PI * Math.pow(pa_tankDiameter, 2)) / 4)
			: pa_tankBottomArea * (pa_legHeight - pa_sludgeDepth); //DOUBLE CHECK 14
  ...
  ---------------------------------------------------------*/
	...
	let p_vaporSpaceExpansionFactor =
		p_avgDailyTempRange / p_avgTemp + p_avgDailyVaporPressueRange / (p_atmosphericPressure - p_vaporPressueStockLiquid); //24
	...
	//Total
	let p_StandingIdleLosses =
		((p_NumDaysIdle * p_vaporSpaceExpansionFactor * (p_vaporPressueStockLiquid * volVaporSpace)) / (p_idealgasConstant * p_avgVaporTemp)) *
		p_vaporMolecularWeight *
		p_standingIdleSaturationFactor; //31

	//Other External Floating Roof Tank
	let p_numberDaysIdle = 1; //36
	let p_tankDiameter = i_tankDiameter; //37
	let p_vaporPressureFunction =
		p_vaporPressueStockLiquid / p_atmosphericPressure / Math.pow(1 + Math.sqrt(1 - p_vaporPressueStockLiquid / p_atmosphericPressure), 2); //39
	let p_windLoss = 0.57 * p_numberDaysIdle * p_tankDiameter * p_vaporPressureFunction * p_vaporMolecularWeight; //40
	let p_totalRoofLandingLosses =
		pa_roofType == "external-floating" && pa_roofShape !== "Dome" ? p_StandingIdleLosses + p_windLoss : p_StandingIdleLosses; //41
  ...
  ---------------------------------------------------------*/
	...
	let r_totalEmissionsLW =
		r_netWorkingLossThroughput *
		r_workingLossSaturationFactor *
		r_workingLossProductFactor *
		r_vaporDensity *
		r_ventSettingCorrectionFactor; //18
	console.log("Net Working Loss Throughput: ", r_netWorkingLossThroughput);
	console.log("Working Loss Saturation Factor: ", r_workingLossSaturationFactor);
	console.log("Working Loss Product Factor: ", r_workingLossProductFactor);
	console.log("Vapor Density: ", r_vaporDensity);
	console.log("Vent Setting Correction Factor: ", r_ventSettingCorrectionFactor);
	console.log("Total Emissions LW: ", r_totalEmissionsLW);
	...
	let r_totalEmissionsLfl =
		((r_trueVaporPressure * r_vaprSpaceVol) / (r_idealGasConstant * r_avgVaporTemp)) *
		r_stockVaporMolecularWeight *
		r_fillingSaturationCorrectionFactorWind *
		r_fillingSaturationFactor;
  ...
  ---------------------------------------------------------*/
	...
	document.getElementById("pumoutTotalEmissions").innerText = Math.round(pr_pumpoutTotalEmissions).toLocaleString();
	document.getElementById("pumpOutGHGImpactLblCO2e").innerText = Math.round(pr_pumpOutGHGImpactLblCO2e).toLocaleString();
	document.getElementById("pumpOutGHGImpactLbltCO2e").innerText = pr_pumpOutGHGImpactLbltCO2e.toFixed(2).toLocaleString();

	document.getElementById("ventilation").innerText = Math.round(pr_ventilation).toLocaleString();
	document.getElementById("ventilationGHGImpactLblCO2e").innerText = Math.round(pr_ventilationGHGImpactLblCO2e).toLocaleString();
	document.getElementById("ventilationGHGImpactLbltCO2e").innerText = pr_ventilationGHGImpactLbltCO2e.toFixed(2).toLocaleString();

	document.getElementById("refilling").innerText = Math.round(pr_refilling).toLocaleString();
	document.getElementById("refillingGHGImpactLblCO2e").innerText = Math.round(pr_refillingGHGImpactLblCO2e).toLocaleString();
	document.getElementById("refillingGHGImpactLbltCO2e").innerText = pr_refillingGHGImpactLbltCO2e.toFixed(2).toLocaleString();

	document.getElementById("totalEmissions").innerText = Math.round(pr_totalEmissions).toLocaleString();
	document.getElementById("totalGHGImpactLblCO2e").innerText = Math.round(pr_totalGHGImpactLblCO2e).toLocaleString();
	document.getElementById("totalGHGImpactLbltCO2e").innerText = pr_totalGHGImpactLbltCO2e.toFixed(2).toLocaleString();
